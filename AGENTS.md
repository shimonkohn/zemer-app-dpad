...
- `.github/workflows/player-monitor.yml` checks hourly: it fetches the live raw `master` file once (the submodule copy is only a warned-about fallback) and decides "known" by running it through `tests/config-covers.mjs` — the harness loader with the app's validation rules — so a pushed-but-invalid entry counts as UNKNOWN and still alerts. Opens an issue + emails, but does **not** auto-commit — the config is added by hand.
